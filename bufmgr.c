			...
			bool *foundPtr)
{
	...
	if (buf_id >= 0)
	{
		...
		*foundPtr = true;
		...
		/*
		* removing buffer from LRU Queue as this buffer being used now
		*/
		if(buf->next != NEXT_NOT_IN_QUEUE)
		{
			removeBufferFromLRU(buf);
			elog(LOG,"Get buffer from LRU Queue %d\n", buf->buf_id);
		}
...
retry:
	...
	elog(LOG, "Added buf to Freelist %d\n", buf->buf_id);
}
...
UnpinBuffer(BufferDesc *buf, bool fixOwner)
{
	...
	if (ref->refcount == 0)
	{
		...
		/* adding buffer to LRU on REFCOUNT becoming zero */
		if(buf->next == NEXT_NOT_IN_QUEUE)
		{
			addBufferToLRU(buf);
			elog(LOG, "Add buf %d\n", buf->buf_id);
